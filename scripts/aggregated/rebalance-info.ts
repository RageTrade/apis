...
import { aave, deltaNeutralGmxVaults, formatUsdc, typechain } from '@ragetrade/sdk'
...
export async function getRebalanceInfo(
  ...
  const { aUsdc, poolAddressProvider } = aave.getContractsSync(networkName, provider)
  ...
  const data = await parallelize(
    ...
    async (_i, blockNumber, event) => {
      ...
      const userData = await aavePool.getUserAccountData(dnGmxJuniorVault.address, {
        blockTag: blockNumber
      })
