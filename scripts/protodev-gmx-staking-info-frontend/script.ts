...
const main = async (PLACEHOLDER_ACCOUNT: any) => {
  ...
  const gmxPrice = poolTokenPriceAmount.mul(ethPrice).div(expandDecimals(1, 18))

  // console.log("gmxPrice", gmxPrice);

  const gmxSupply = await (await fetchRetry(getServerUrl(chainId, '/gmx_supply'))).text()
...
export const getGmxData = async () => {
  ...
  const stakedGlpApr = 100 * ((1 + data.glpAprForNativeToken / (100 * 365)) ** 365 - 1)
  // console.log("stakedGlpApr", stakedGlpApr);
  const esGmxApr = 100 * ((1 + data.gmxAprForNativeToken / (100 * 365)) ** 365 - 1)
