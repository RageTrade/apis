...
export async function perInterval2(networkName: NetworkName) {
  ...
  const startBlock = 55725219
  const endBlock = await provider.getBlockNumber()
  const interval = 600

  const data = await parallelize(
    {
      label: 'per-interval-5',
      networkName,
      provider,
      getEvents: () => {
        const events = []
    ...
    async (_i, blockNumber) => {
      ...
      const wethGlobalShortAveragePrices = Number(
        formatUnits(
          await gmxUnderlyingVault
            .attach(shortsTrackerAddress)
            .globalShortAveragePrices(weth.address, {
              blockTag: blockNumber
            }),
          30
        )
      ...
      const wbtcGlobalShortAveragePrices = Number(
        formatUnits(
          await gmxUnderlyingVault
            .attach(shortsTrackerAddress)
            .globalShortAveragePrices(wbtc.address, {
              blockTag: blockNumber
            }),
          30
        )
