...
import { gmxProtocol, NetworkName, deltaNeutralGmxVaults, tokens, typechain } from "@ragetrade/sdk";
...
export const getEsgmxRewards = async (networkName: NetworkName) => {
  ...
  const netEsGmxBal = Number(formatEther(await esGmx.balanceOf(dn.dnGmxJuniorVault.address)))
  const netGlpBal = (await tk.sGLP.balanceOf(dn.dnGmxBatchingManager.address)).add(await tk.sGLP.balanceOf(dn.dnGmxJuniorVault.address))

  let netGlpInUsd = netGlpBal.mul(glpPrice).mul(BASIS_POINTS_DIVISOR).div(PRICE_PRECISION).toNumber()
  ...
  const adjustedEsGmxApy = vmv > 0 ? ((esGmxApyforGlp * netGlpInUsd) + (esGmxApyforGmx * netEsGmxBal * gmxPrice)) / vmv : 0
