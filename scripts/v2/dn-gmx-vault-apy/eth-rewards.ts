...
import { chainlink, deltaNeutralGmxVaults, formatUsdc, gmxProtocol } from '@ragetrade/sdk'
...
export const getEthRewards = async (networkName: NetworkName) => {
  ...
  const { sGLP, glp } = await gmxProtocol.getContractsSync(networkName, provider)
  ...
  const tokensPerInterval = Number(formatEther(await feeGlpTracker.tokensPerInterval()))
  const SECONDS_PER_YEAR = 31536000

  const feeGlpTrackerAnnualRewardsUsd = tokensPerInterval * SECONDS_PER_YEAR * ethPrice
  ...
  const jrTvl = Number(formatUsdc(await dnGmxJuniorVault.getVaultMarketValue()))
  const srTvl = Number(formatUsdc(await dnGmxSeniorVault.getVaultMarketValue()))
