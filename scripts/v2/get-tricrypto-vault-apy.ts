...
import { CacheServerDataSource, formatUsdc, tricryptoVault } from '@ragetrade/sdk'
...
const TRICRYPTO_POOL_ADDRESS = '0x960ea3e3c7fb317332d990873d354e18d7645590'.toLowerCase()
...
async function getApyEstimation(
  ...
  const rageAPY = safeDivNumer(earnings * 365, Number(formatUsdc(avgVaultMarketValueD6)))
...
export async function getVaultLpFeesEarnings(
  ...
  const res = await graphqlClient
    .query<VaultRebalanceQuery, VaultRebalanceQueryVariables>(VaultRebalanceDocument, {
  ...
  if (rebalances?.[0] && rebalances?.[1]) {
    const lpFeesInLastRebalance = Number(rebalances[0].liquidityPositionEarningsRealized)
