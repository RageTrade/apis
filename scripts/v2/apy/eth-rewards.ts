...
const rageSubgraphUrl = 'https://api.thegraph.com/subgraphs/name/fr0ntenddev/rage-trade'

const queryRewardsData = async (vault_id: string, from_ts: string, to_ts: string, first: number, skip: number) => {
...
const queryVaultMktValueData = async (vault_id: string, from_ts: string, to_ts: string, first: number, skip: number) => {
...
export const getEthRewards = async (networkName: NetworkName) => {
  ...
  const vaultMktValueData = await queryVaultMktValueData(vault_id, from_ts, to_ts, 1000, 0)

  const ethPrice = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd')).json()).ethereum.usd
  ...
  juniorVaultAvgVmv = vaultMktValueData > 0 ? juniorVaultAvgVmv / vaultMktValueData.vault.rebalances.length : 0

  // TODO: replace "vaultMktValueData.vault.rebalances.length" with appropriate value for senior tranche
  seniorVaultAvgVmv = vaultMktValueData > 0 ? seniorVaultAvgVmv / vaultMktValueData.vault.rebalances.length : 0

  const jrVaultRewardsApy = juniorVaultAvgVmv > 0 ? (juniorVaultRewards * ethPrice * weeks) / juniorVaultAvgVmv : 0
  const srVaultRewardsApy = seniorVaultAvgVmv > 0 ? (seniorVaultRewards * ethPrice * weeks) / seniorVaultAvgVmv : 0
