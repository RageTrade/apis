...
export async function getBlockByTimestamp(
  ...
  if (networkName === 'arbmain') {
    try {
      return Number(
        (
          await (
            await fetchRetry(`https://coins.llama.fi/block/arbitrum/${timestamp}`)
          ).json()
        ).height
      )
    } catch {}
  ...
  if (networkName === 'arbgoerli') {
    const block = await findBlockByTimestamp(getProvider('arbgoerli'), timestamp, {
  ...
  while (1) {
    ...
    if (JSON.stringify(resp).includes('Max rate limit reached')) {
      await new Promise((res) => setTimeout(res, 1000 + Math.floor(Math.random() * 100)))
      debug('Arbiscan retry')
      continue // try again
    } else {
      throw new Error(
        `Arbiscan Api Failed. Request: ${request}. Response: ${JSON.stringify(resp)}`
      )
    }
  }
